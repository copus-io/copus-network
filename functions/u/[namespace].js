// Cloudflare Worker for Short User Profile URL (/u/namespace)
// Same as /user/[namespace] but for the short URL format
...
function buildUserJsonResponse(user, treasuries) {
  const profileUrl = `${SITE_URL}/u/${user.namespace}`
  ...
  return {
    ...
    url: profileUrl,
    canonicalUrl: `${SITE_URL}/user/${user.namespace}`,
    bio: user.bio || null,
...
class BodyInjector {
  ...
  element(element) {
    ...
    const personSchema = {
      "@context": "https://schema.org",
      "@type": "Person",
      "@id": `${SITE_URL}/user/${user.namespace}#person`,
      "name": name,
      "url": profileUrl,
      ...
      "sameAs": [`${SITE_URL}/user/${user.namespace}`],
    ...
    // Add treasuries as owned collections
    ...
    const ssrContent = `
      <div id="copus-ssr-profile" style="position:absolute;left:-9999px;top:0;width:1px;height:1px;overflow:hidden;">
        <header>
          ...
          <p><strong>Stats:</strong> ${articleCount} curations created, ${collectedCount} items collected</p>
