...
export const Login = (): JSX.Element => {
  ...
  useEffect(() => {
    const handleOAuthCallback = async () => {
      ...
      if (code && state) {
        console.log(`🔗 处理 OAuth 回调，provider: ${provider}, code:`, code, 'state:', state);
        ...
        try {
          ...
          if (provider === 'facebook') {
            ...
            if (response.isBinding) {
              // 账号绑定模式
              console.log('✅ Facebook 账号绑定成功，响应:', response);
              ...
              // 第三方登录模式
              console.log('✅ Facebook 登录成功，响应:', response);
            ...
            if (response.isBinding) {
              // 账号绑定模式
              console.log('✅ Google 账号绑定成功，响应:', response);
              ...
              // 第三方登录模式
              console.log('✅ Google 登录成功，响应:', response);
            ...
            console.log('✅ X 登录成功，响应:', response);
  ...
  const handleSocialLogin = async (provider: string) => {
    ...
    if (provider === 'X') {
      ...
      try {
        ...
        if (token) {
          // 已登录用户，使用API获取OAuth URL（账号绑定）
          console.log('🔗 已登录用户，获取Facebook绑定URL');
          ...
          // 未登录用户，使用手动构建的OAuth URL（第三方登录）
          console.log('🔗 未登录用户，使用手动构建的Facebook登录URL');
      ...
      try {
        ...
        if (token) {
          // 已登录用户，使用API获取OAuth URL（账号绑定）
          console.log('🔗 已登录用户，获取Google绑定URL');
          ...
          // 未登录用户，使用手动构建的OAuth URL（第三方登录）
          console.log('🔗 未登录用户，使用手动构建的Google登录URL');
      ...
      try {
        ...
        if (response.isBinding) {
          // 账号绑定模式
          console.log('✅ Metamask 账号绑定成功，响应:', response);
          ...
          // 第三方登录模式
          console.log('✅ Metamask 登录成功，响应:', response);
