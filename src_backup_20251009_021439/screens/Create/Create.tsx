...
export const Create = (): JSX.Element => {
  ...
  useEffect(() => {
    const loadArticleForEdit = async () => {
      ...
      try {
        console.log('📝 加载文章数据进行编辑:', editId);
        const articleData = await getArticleDetail(editId);
        console.log('✅ 获取到文章数据:', articleData);
    ...
    try {
      ...
      if (formData.coverImage) {
        try {
          ...
          console.log('✅ 图片上传成功，URL:', finalCoverUrl);
      ...
      console.log('🚀 准备' + (isEditMode ? '更新' : '发布') + '文章:', articleParams);

      const response = await publishArticle(articleParams);
      console.log('✅ 文章' + (isEditMode ? '更新' : '发布') + '成功:', response);
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <Badge
                      key={category.id}
                      variant="outline"
                      style={isSelected ? getCategoryInlineStyle(category.color) : undefined}
