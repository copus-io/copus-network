...
export const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  const handleUserClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (onUserClick && article.userId) {
      onUserClick(article.userId);
    }
  };
    ...
    switch (layout) {
      ...
      default:
        return (
          <CardContent className="flex flex-col gap-[25px] py-5 px-[30px] flex-1">
            <div className="flex flex-col gap-5 flex-1">
              <div className="relative h-[240px] rounded-lg overflow-hidden bg-gray-200 cursor-pointer transition-transform hover:scale-[1.02]">
                <img
