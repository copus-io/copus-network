...
export const NotificationListSection = (): JSX.Element => {
  return (
    <section className="flex flex-col items-start gap-2.5 pl-[60px] pr-10 py-0 flex-1 grow translate-y-[-1rem] animate-fade-in opacity-0">
        ...
        <TabsList className="grid w-full grid-cols-3 bg-transparent h-auto p-0 gap-0">
          {notificationTabs.map((tab) => (
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className={`flex-1 justify-center px-[15px] py-2.5 data-[state=active]:border-b data-[state=active]:border-[#454545] data-[state=active]:bg-transparent data-[state=active]:shadow-none rounded-none bg-transparent ${
                tab.value === "all"
        ...
        <TabsContent value="all" className="mt-5">
          <div className="flex flex-col gap-5 pb-[30px]">
            {notifications.map((notification, index) => (
              <Card
                key={notification.id}
                className={`p-0 border-0 rounded-lg border-b border-white translate-y-[-1rem] animate-fade-in opacity-0 ${
