...
export class AuthService {
  ...
  /**
   * Get Kite AI OAuth authorization URL (supports both login and binding modes)
   */
  static async getKiteOAuthUrl(): Promise<string> {
    const endpoint = `/client/common/kite/oauth`;
    ...
    try {
      ...
      throw new Error('Did not receive a valid Kite OAuth URL');
    } catch (error) {
      console.error('Failed to get Kite OAuth URL:', error);
      throw new Error(`Failed to get Kite OAuth URL: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  /**
   * Kite AI login callback handler (supports both login and binding modes)
   */
  static async kiteLogin(code: string, state: string, hasToken: boolean = false): Promise<any> {
    const endpoint = `/client/common/kite/login?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`;
    ...
    try {
      if (hasToken) {
        ...
        const kiteProfile = {
          username: response.data?.username || response.username || response.data?.name || response.name,
          faceUrl: response.data?.faceUrl || response.faceUrl || response.data?.avatar || response.avatar,
          email: response.data?.email || response.email
        };
      ...
      console.error('Kite Login/Binding failed:', error);
      throw new Error(`Kite ${hasToken ? 'account binding' : 'login'} failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
