...
export const AnalyticsPanel: React.FC<AnalyticsPanelProps> = ({ isOpen, onClose }) => {
  ...
  const tabs = [
    ...
    { id: 'publish', label: 'Publish Buttons' },
    { id: 'funnel', label: 'Creation Funnel' },
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh]">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  <h3 className="text-sm font-medium text-blue-800">Total Button Clicks</h3>
                  <p className="text-2xl font-bold text-blue-900">{analytics.publishStats.totalClicks}</p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  <h3 className="text-sm font-medium text-green-800">Page Access</h3>
                ...
                <div className="bg-orange-50 p-4 rounded-lg">
                  <h3 className="text-sm font-medium text-orange-800">Unique Sessions</h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h4 className="font-medium mb-3">Creation Funnel Overview</h4>
          ...
          {activeTab === 'publish' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-medium mb-4">Clicks by Location</h4>
                ...
                <div>
                  <h4 className="font-medium mb-4">Clicks by User Type</h4>
                  <div className="space-y-3">
                    {Object.entries(analytics.publishStats.clicksByUserType).map(([userType, clicks]) => (
                      <div key={userType} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                        <span className="capitalize">{userType.replace('_', ' ')}</span>
          ...
          {activeTab === 'funnel' && (
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-blue-50 to-green-50 p-6 rounded-lg">
                <h4 className="font-medium mb-4">Creation Process Funnel</h4>
                <div className="space-y-4">
                  {[
                    { label: 'Page Loads', value: analytics.creationFunnel.pageLoads, color: 'bg-blue-500' },
                    { label: 'Title Inputs', value: analytics.creationFunnel.titleInputs, color: 'bg-indigo-500' },
                    { label: 'Content Inputs', value: analytics.creationFunnel.contentInputs, color: 'bg-purple-500' },
                    { label: 'Category Selects', value: analytics.creationFunnel.categorySelects, color: 'bg-pink-500' },
                    { label: 'Publish Attempts', value: analytics.creationFunnel.publishAttempts, color: 'bg-orange-500' },
                    { label: 'Published Success', value: analytics.creationFunnel.publishSuccess, color: 'bg-green-500' }
                  ].map((step) => {
              ...
              <div>
                <h4 className="font-medium mb-4">Drop-off Rates</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(analytics.creationFunnel.dropOffRates).map(([step, rate]) => (
                    <div key={step} className="bg-red-50 p-3 rounded border-l-4 border-red-400">
                      <div className="font-medium text-red-800">{step.replace(/([A-Z])/g, ' $1').trim()}</div>
          ...
          {activeTab === 'journey' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                  <h4 className="font-medium mb-4">User Retention</h4>
                  <div className="space-y-4">
          ...
          {activeTab === 'time' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    {analytics.timeAnalytics.peakHours.map((hour) => (
                      <div key={hour} className="flex justify-between p-2 bg-blue-50 rounded">
                        ...
                        <span className="font-medium">{analytics.timeAnalytics.hourlyDistribution[hour] || 0} events</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <h4 className="font-medium mb-4">Peak Days</h4>
                  <div className="space-y-2">
                    {analytics.timeAnalytics.peakDays.map((day) => (
                      <div key={day} className="flex justify-between p-2 bg-green-50 rounded">
                        <span>{getDayName(day)}</span>
                        <span className="font-medium">{analytics.timeAnalytics.weeklyDistribution[day] || 0} events</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <h4 className="font-medium mb-4">Hourly Activity Distribution</h4>
