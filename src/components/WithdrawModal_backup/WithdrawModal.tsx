...
import { EmailBindingPrompt } from "../EmailBindingPrompt/EmailBindingPrompt";
      ...
      >
        ...
        <div className="flex flex-col w-[518px] items-center gap-[25px] px-8 py-0 relative flex-[0_0_auto] ml-[-29.00px] mr-[-29.00px]">
          <header className="flex flex-col items-center justify-center relative self-stretch w-full flex-[0_0_auto]">
            ...
            <h1
                id="withdraw-title"
                className="relative w-fit [font-family:'Lato',Helvetica] font-semibold text-gray-900 text-2xl tracking-[0] leading-9 whitespace-nowrap mx-auto"
              >
                {showEmailBinding ? "🔐 邮箱绑定验证" : showEmailVerification ? "📧 邮箱验证" : "💸 提现申请"}
              </h1>
              {!showEmailBinding && !showEmailVerification && (
                <p className="text-sm text-gray-500 mt-2">快速、安全、透明的提现服务</p>
              )}
          </div>
          </header>
          ...
          {showEmailBinding && (
            <EmailBindingPrompt
              ...
              onUpdateEmail={async (newEmail: string) => {
                // 更新邮箱地址
                updateUser({ email: newEmail, emailVerified: false });
                return true;
              }}
              onCancel={() => {
                setShowEmailBinding(false);
          ...
          {/* 提现表单界面 */}
          {!showEmailVerification && !showEmailBinding && (
          <div>
          {/* 进度指示器 */}
          <div className="w-full mb-4">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className="bg-gradient-to-r from-blue-500 to-blue-600 h-2 rounded-full transition-all duration-500 w-1/3"></div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 p-3 relative self-stretch w-full rounded-lg bg-blue-50 border border-blue-200">
            ...
            <div className="text-xs text-blue-800 [font-family:'Lato',Helvetica]">
              ...
              <div className="space-y-0.5">
                <div>• 每日快速提现，收取 1 USD 手续费，1-3分钟到账</div>
                <div>• 比YouTube快30倍，比微信公众号快7倍</div>
                <div>• 最低提现: {MIN_WITHDRAWAL_AMOUNT} {displayCurrency}，当日最高: 10,000 {displayCurrency}</div>
                <div>• 需验证绑定邮箱，确保资金安全 🔐</div>
            ...
            >
              ...
              <span className={`relative w-fit [font-family:'Lato',Helvetica] font-bold text-lg tracking-[0] leading-5 whitespace-nowrap ${
                ...
                {isSubmitting ? "处理中..." :
                 ...
                 !isMinimumAmount ? `最低提现 ${MIN_WITHDRAWAL_AMOUNT} ${displayCurrency}` :
