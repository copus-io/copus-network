...
export interface ArticleData {
  ...
  // Earnings preview fields (for my share cards)
  totalEarnings?: number; // Total earnings from this article in USDC
  monthlyEarnings?: number; // This month's earnings in USDC
  isEarningsPreview?: boolean; // Whether to show earnings preview
}
...
export interface ActionConfig {
  ...
  showWebsite?: boolean;
  showEarningsPreview?: boolean; // Show earnings preview panel
}
      ...
      default:
        return (
          <CardContent className="flex flex-col gap-[20px] py-5 px-[30px] flex-1">
            ...
            {/* Earnings Preview Panel - only show on hover for my share cards */}
            {isHovered && (actions.showEarningsPreview || article.isEarningsPreview) && (article.totalEarnings !== undefined || article.monthlyEarnings !== undefined) && (
              <div className="border-t border-gray-100 -mx-[30px] px-[30px] pt-4 mt-2 animate-in fade-in duration-200">
                <div className="bg-gradient-to-r from-green-50 via-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-green-600">💰</span>
                      <span className="text-sm font-medium text-green-800">收益预览</span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {article.monthlyEarnings !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-bold text-green-700">
                          ${article.monthlyEarnings.toFixed(2)}
                        </div>
                        <div className="text-xs text-green-600">本月收益</div>
                      </div>
                    )}

                    {article.totalEarnings !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-bold text-green-700">
                          ${article.totalEarnings.toFixed(2)}
                        </div>
                        <div className="text-xs text-green-600">总收益</div>
                      </div>
                    )}
                  </div>
                </div>
