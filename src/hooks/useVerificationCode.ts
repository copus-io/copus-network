...
export const useVerificationCode = (props?: UseVerificationCodeProps) => {
  ...
  const sendCode = useCallback(
    async (email: string, codeType: number) => {
      // If countdown is active or sending in progress, ignore request
      ...
      try {
        ...
        if (success) {
          ...
          props?.onSendError?.('Failed to send verification code, please try again later');
          return false;
        }
      } catch (error) {
        console.error('Failed to send verification code:', error);
        props?.onSendError?.('Network error, please check your connection and try again');
        return false;
      } finally {
        setIsSending(false);
      }
