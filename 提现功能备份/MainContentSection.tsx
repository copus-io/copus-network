...
export const MainContentSection = (): JSX.Element => {
  ...
  const transformApiToCard = (article: any): ArticleData => {
    return {
      ...
      earnings: generateArticleEarnings(article.uuid), // 为每篇文章生成收益数据
      isLiked: false,
      targetUrl: article.targetUrl,
    ...
    return (
      <ArticleCard
        ...
        actions={{
          ...
          showEarnings: !isViewingOtherUser // 只有查看自己的页面才显示收益
        }}
  ...
  // 处理提现
  const handleWithdraw = (amount: number, walletAddress: string) => {
    console.log('提现申请:', { amount, walletAddress });
    // 模拟提现成功 - 无手续费，实际到账金额等于提现金额
    setEarningsData(prev => ({
      ...prev,
      availableBalance: prev.availableBalance - amount,
      withdrawnAmount: prev.withdrawnAmount + amount
    }));
    showToast(`提现申请已提交！实际到账: ${amount.toFixed(2)} USDC (免手续费)`, 'success');
  };
